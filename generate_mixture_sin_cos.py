...
class sin_cos_mixture:
    ...
    def conditional_generate(self, x_given, N_sample: int):
        """
        Conditional sampling for the mixture on [-1,1]^dim.

        Input:
          x_given : array-like, shape (s,)
              Observed prefix (x_1, ..., x_s). Must satisfy 0 <= s <= dim.
          N_sample : int
              Number of conditional samples.

        Output:
          X : np.ndarray, shape (N_sample, dim)
              Samples from p(x_{1:dim} | x_{1:s} = x_given),
              with the first s coordinates fixed to x_given.
        """
        ...
        if s > self.dim:
            raise ValueError(f"x_given length s={s} cannot exceed dim={self.dim}.")
        if N_sample == 0:
            return np.empty((0, self.dim), dtype=float)

        # Support check (both components are supported on [-1,1])
        if np.any(x_given < -1.0) or np.any(x_given > 1.0):
            raise ValueError("All entries of x_given must be in [-1,1].")

        # If fully observed, conditional is a point mass.
        if s == self.dim:
            X = np.tile(x_given[None, :], (N_sample, 1))
            return X

        # Compute posterior mixture weights:
        #   w_k(x_given) ∝ w_k * Π_{j<=s} f_k(x_j)
        # Use logs for stability.
        eps = 1e-300  # avoids log(0) numerical issues

        if s == 0:
            post_w = self.weights.copy()
        else:
            f0 = self.sin_comp._pdf1(x_given)  # shape (s,)
            f1 = self.cos_comp._pdf1(x_given)  # shape (s,)

            loglik0 = np.sum(np.log(np.maximum(f0, eps)))
            loglik1 = np.sum(np.log(np.maximum(f1, eps)))

            logw0 = np.log(self.weights[0]) + loglik0
            logw1 = np.log(self.weights[1]) + loglik1
            ...
            if denom <= 0.0 or not np.isfinite(denom):
                raise ValueError("Conditioning event has (numerically) zero probability under both components.")
            post_w = np.array([a0 / denom, a1 / denom], dtype=float)

        # Sample component labels using posterior weights
        z = np.random.choice(2, size=N_sample, p=post_w)
        n0 = int(np.sum(z == 0))
        n1 = N_sample - n0

        # Sample remaining coordinates
        r = self.dim - s
        X = np.empty((N_sample, self.dim), dtype=float)
        if s > 0:
            X[:, :s] = x_given[None, :]

        if n0 > 0:
            tail0 = self.sin_comp._sample_1d(n0 * r).reshape(n0, r)
            X[z == 0, s:] = tail0
        if n1 > 0:
            tail1 = self.cos_comp._sample_1d(n1 * r).reshape(n1, r)
            X[z == 1, s:] = tail1
